...
The `cut.default` function beautifully enunciates
...
The results is a labeling of the levels according to the 
...
Here the levels are labeled with the last day of each month,
...
and the date is displayed in the default format of the author's locale.
```{r}
...
We will show how to use the first and third.
...
The level labels can be automatically abbreviated as follows:
```{r}
...
```
ABC is interested in calculating the ages in "months" of those amounts as of the
...
```
^[R includes the units in FYage because 'age' is a `difftime` object.]

Barplots of total loss by fiscal year and by fiscal year age can be
...
* `mondate(DateOfLoss)` is just one convenient way of abbreviating the factor levels.
...
Many thanks to the R Development team for their work on
...
Mr. Laker goes on to suggest two cases:
...
The mondate package embraces Mr. Laker's month-centric, end-of-business perspective.
