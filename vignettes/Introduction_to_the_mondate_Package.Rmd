...
These two calculations yield the same result?
```{r}
...
those occurring in 2015.
That moment is exactly one year away from the corresponding
moment, 
`mondate("2015-12-31")`,
...
(m <- seqmondate(as.Date("2015-01-31"), length = 5))
class(d)
class(m)
```
...
`FALSE`.^[Excluding the minimum value of 'x' would be somewhat "random" given
that other values of 'x' are probably in the same time interval.
In the chase of `character` breakls,
`include.lowest=FALSE` will result in an error.]
Other `cut` arguments have default values as well that may seem 
counterintuitive
when it comes to cutting dates.
Probably the most troubling default is `right = TRUE` for `Date` objects 
because it violates 
...
Other arguments were added as well, which we will see in due course.
...
Here we show three ways to cut DateOfLoss.
The choice may depend on whether the company thinks of its fiscal year
as being identified with the beginning or ending calendar year 
(right = FALSE or TRUE, respectively).
...
ABC's actuaries want to calculate the "fiscal-accident-year" ages 
in "months" of those amounts as of
June 30, 2016,
and total thos amounts by fiscal-accident-year.
No problem!

First, cut the loss dates into fiscal years (FY), 
but this time also use
"attr.breaks = TRUE",
which makes the break points available for subsequent age calculations.
```{r}
...
```
^[<span style="color:blue; font-family:Georgia;">R</span>
includes the units in FYage because 'age' is a `difftime` object.]

Then plot the loss totals.
The first plot below is by FY, the second by the age of the FY.
```{r}
...
Mr. Laker goes on to state his Recommended Method based on two cases:
