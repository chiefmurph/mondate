...
```{r time_interval_int, fig.width = 10, fig.height=2, echo = F, fig.cap="time intervals A and B"}
...
```{r rotated_5_days, fig.width = 10, fig.height=2, echo = F, fig.cap="Calendar days surrounding January 1, 1970, time interval borders modeled by POSIXt instance `a`"}
#plot.window(xlim=c(0,5), ylim=c(-.5, .5))
par(mar=c(1,2,1,2))
par(lend="butt")
NDaysInSecs <- 60*60*24*n
