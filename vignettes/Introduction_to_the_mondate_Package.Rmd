...
let's first visit some "Basic Concepts" discussed by ISO 8601's:
time intervals and their characteristics, including units of measurement.
...
Here is a diagram of three time intervals A, B, and C.
Each of the intervals include their respective endpoints
as allowed in the definiton,
as indicated by the square brackets "[" and "]".
When an endpoint is included in an interval, that
endpoint is called "closed."
When not included, the endpoint is called "open."
Note that intervals B and C overlap at time instant 4 because
each are closed at that point.
...
text(x=1.013, y=0, "[", col="blue", font = 2)
text(x=1.987, y=0, "]", col="blue", font = 2)
text(x=1.5, y= .1, "A", col="blue")

segments(x0=3.01, y0=0, x1=3.99, col="blue", lwd =4)
text(x=3.013, y=0, "[", col="blue", font = 2)
text(x=3.987, y=0, "]", col="blue", font = 2)
text(x=3.5, y= .1, "B", col="blue")

segments(x0=4.01, y0=0, x1=4.99, col="blue", lwd =4)
text(x=4.01, y=0, "[", col="blue", font = 2)
text(x=4.987, y=0, "]", col="blue", font = 2)
...
For example, suppose Figure 1 illustrates
the first five days of calendar year 1970.
Intervals A, B, and C correspond to January 2, 4, and 5, respectively.
ISO 8601 recognizes that all times within January 2nd may be 
labeled with the mark "January 2, 1970."
Other "marks" that may be used are "1970-01-02" and "1970-01-02 00:00:00".
Indeed, in this case the mark "1970-01-02 00:00:00",
i.e., midnight between January 1st and 2nd, 
refers not just to that instant but as well to every other
instant during January 2, 1970.
ISO 8601 recognizes this common tradition when it says
"On a time scale consisting of successive steps, 
two distinct instants may be expressed by the same time 
point."^[2.1.5 Note 1]

On the other hand, if B and C are allowed to overlap 
at the instant "1970-01-05 00:00:00" 
then that mark -- and its commonly-shortened version "1970-01-05" -- 
could conceivably express all instants within both B
(January 4) and C (January 5), 
clearly an undesirable situation.

To avoid overlapping time intevals it is preferable to leave 
one of the endpoints "open", indicated by parens:
"(" for a left-open endpoint, ")" for a right-open endpoint.

R's POSIXt implementation models time intervals as left-closed,
...
using the `POSIXct` class to represent the endpoints.
...
for (i in 1:(2*n)) text(x=a[i] + 15*60, y=0, "[", col="blue", font = 2)
for (i in 2:(2*n+1)) text(x=a[i] - 15*60, y=0, ")", col="blue", font = 2)
...
```{r time_interval_open, fig.width = 10, fig.height=2, echo = F, fig.cap="a left-closed / right-open time interval"}
#plot.window(xlim=c(0,5), ylim=c(-.5, .5))
par(mar=c(1,2,1,2))
par(lend="butt")
plot(c(2,3), c(0,0), col="blue", type = "l",
...
axis(1, pos=-.2)
axis(1, pos=.3)
arrows(x0=-.1, y0=-.2, x1=5.15, length=.1)
text(5.1, 0, "t", pos = 1)
segments(x0=2.01, y0=0, x1=2.99, col="blue", lwd =4)
text(x=2.013, y=0, "[", col="blue", font = 2)
text(x=2.987, y=0, ")", col="blue", font = 2)
box()
```
